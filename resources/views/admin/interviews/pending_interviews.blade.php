...
<div id="loader">
  ...
  <h6 class="fw-semibold mb-0">Pending Interviews</h6>

</div>
    
    <div class="card basic-data-table">
      <div class="card-header">
        <!--<h5 class="card-title mb-0">Pending Interviews-->
        <!--</h5>-->
        <br>
  ...
  <div class="row g-3">
    ...
    <div class="col-md-3">
      ...
      <select name="qualifications[]" class="select2" multiple>
            <option value="">Choose</option>
            @foreach(["Bachelor's", "Master's", "PhD", "High School", "Plus Two","Diploma"] as $qualification)
        ...
        <div class="modal-content p-3">
          ...
          <div class="d-flex justify-content-around">
             <div class="modal-body text-center">
                <a href="{{ route('interview.students.export',array_merge(['type' => 'pdf','status'=> 'Pending'], request()->all())) }}" class="btn btn-danger">Export as PDF</a>
                <a href="{{ route('interview.students.export', array_merge(['type' => 'excel','status'=> 'Pending'], request()->all())) }}" class="btn btn-success">Export as Excel</a>
              </div>
          </div>
        </div>
      </div>
...
@foreach($interviews as $e)
            <tr>
              ...
              <td>{{$e->GetStd->first_name}} {{$e->GetStd->middle_name}} {{$e->GetStd->last_name}} <br><small>{{$e->GetStd->mobile_code}}{{$e->GetStd->mobile}}<br>{{$e->GetStd->mobile_code}}{{$e->GetStd->alternate_mobile}}</small></td>
              <!--<td></td>-->
...
@endforeach
  ...
  $('.row-checkbox:checked').each(function () {
    ...
    var name = row.find('td').eq(2).clone().children().remove().end().text().trim();
    var jobRole = row.find('td').eq(6).text();
console.log(name);
